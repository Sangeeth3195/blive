...
public class FragmentGlobalWeek extends BaseFragment implements AdapterGlobal.Listener {
    ...
    private void initUI() {
        ...
        swipeRefreshLayout = mActivity.findViewById(R.id.swipeRefreshGlobalWeek);
                ...
                public void onResponse(@NonNull Call<LeaderBoardResponse> call, @NonNull Response<LeaderBoardResponse> response) {
                    ...
                    if (leaderBoardResponse != null) {
                        if (leaderBoardResponse.getStatus().equalsIgnoreCase("success")) {
                            setData(leaderBoardResponse.getData().getWeeklyUsers());
                        }
                    } else {
    ...
    private void setData(ArrayList<User> mWeeklyUsers) {
        weeklyUsers = mWeeklyUsers;
        if (weeklyUsers.size() > 0) {
            int size = weeklyUsers.size();
            if (size < 4) {
                switch (size) {
                    case 1:
                        ...
                        tv_name.setText(weeklyUsers.get(0).getName());
                        tv_received.setText(weeklyUsers.get(0).getGift_value());
                        tv_level.setText("Lv :" + weeklyUsers.get(0).getLevel() + " ");
                        break;
                    case 2:
                        id1.setVisibility(View.VISIBLE);
                        ...
                        tv_name_1.setText(weeklyUsers.get(1).getName());
                        tv_received_1.setText(weeklyUsers.get(1).getGift_value());
                        tv_level_1.setText(" Lv :" + weeklyUsers.get(1).getLevel() + " ");
                        break;
                    case 3:
                        id1.setVisibility(View.VISIBLE);
                        id2.setVisibility(View.VISIBLE);
                        ...
                        tv_name_2.setText(weeklyUsers.get(2).getName());
                        tv_received_2.setText(weeklyUsers.get(2).getGift_value());
                        tv_level_2.setText(" Lv :" + weeklyUsers.get(2).getLevel() + " ");
                        break;
                }
            } else {
                id1.setVisibility(View.VISIBLE);
                ...
                for (int i = 3; i < weeklyUsers.size(); i++) {
                    users.add(weeklyUsers.get(i));
                }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_global_week, container, false);
    }
    ...
    private void displayProfilePic(int size) {
        for (int i = 0; i < size; i++) {
            ...
            linearLayouts.get(i).setOnClickListener(view -> {
                if (!SessionUser.getUser().getUser_id().equals(weeklyUsers.get(finalI).getUser_id())) {
                    Intent intent = new Intent(mActivity, ActivityViewProfile.class);
                    intent.putExtra("image", weeklyUsers.get(finalI).getProfile_pic());
                    intent.putExtra("userId", weeklyUsers.get(finalI).getUser_id());
                    intent.putExtra("from", "list");
                    startActivity(intent);
                }
            });

            String image = weeklyUsers.get(finalI).getProfile_pic();
